<?php
    ...
    $consultaasc = $bd->prepare("SELECT `id`, `tarea` FROM `tareas` WHERE `estado` = ? ORDER BY `prioridad` ASC"); 
    //Se prepara la sentencia para mostrat las tareas finalizadad
    //Ordenadas de la A a la Z por el contenido de la tarea 
    $consultadesc = $bd->prepare("SELECT `id`, `tarea` FROM `tareas` WHERE `estado` = ? ORDER BY `prioridad` DESC");
